...
import numpy as np
import cv2
import threading  # ★ 추가: SAM2 동시성 제어용 전역 락

# ★ 전역 SAM 락: 모든 Agent 인스턴스가 공유
_SAM_GLOBAL_LOCK = threading.Lock()
...
class BodyPart:
    ...
    @classmethod  ### 6
    def left_lower_arm(cls):  return ['arm', cls.LEFT_ELBOW, cls.LEFT_WRIST]
    @classmethod  ### 10
    def left_upper_arm(cls):  return ['arm', cls.LEFT_ELBOW, cls.LEFT_SHOULDER]
    @classmethod  ### 15
    def right_lower_arm(cls): return ['arm', cls.RIGHT_ELBOW, cls.RIGHT_WRIST]
    @classmethod  ### 19
    def right_upper_arm(cls): return ['arm', cls.RIGHT_ELBOW, cls.RIGHT_SHOULDER]
    @classmethod  ### 7
    def left_lower_leg(cls):  return ['leg', cls.LEFT_ANKLE, cls.LEFT_KNEE]
    @classmethod  ### 11
    def left_upper_leg(cls):  return ['leg', cls.LEFT_KNEE, cls.LEFT_HIP]
    @classmethod  ### 16
    def right_lower_leg(cls): return ['leg', cls.RIGHT_ANKLE, cls.RIGHT_KNEE]
    @classmethod  ### 20
    def right_upper_leg(cls): return ['leg', cls.RIGHT_KNEE, cls.RIGHT_HIP]
...
class BodyPartValue:
    def __init__(self):
        ...
        # points
        self.left_upper_arm_point = None
        self.right_upper_arm_point = None
        self.left_lower_arm_point = None
        self.right_lower_arm_point = None
        self.left_upper_leg_point = None
        self.right_upper_leg_point = None
        self.left_lower_leg_point = None
        self.right_lower_leg_point = None
        self.shoulder_point = None
        self.waist_point = None

    def get_data(self):
        return {'length': [
            self.left_upper_arm_width, self.right_upper_arm_width,
            self.left_lower_arm_width, self.right_lower_arm_width,
            self.left_upper_leg_width, self.right_upper_leg_width,
            self.left_lower_leg_width, self.right_lower_leg_width,
            self.shoulder_width, self.waist_width
        ], 'points': [
            self.left_upper_arm_point, self.right_upper_arm_point,
            self.left_lower_arm_point, self.right_lower_arm_point,
            self.left_upper_leg_point, self.right_upper_leg_point,
            self.left_lower_leg_point, self.right_lower_leg_point,
            self.shoulder_point, self.waist_point
        ]}
...
class Agent:
    ...
    def get_outline_mask(self, img=None):
        """
        원본과 동일한 시그니처/리턴.
        내부에서 SAM2 사용 구간만 전역락으로 보호하여 set_image 레이스를 차단.
        """
        if img is not None:
            with _SAM_GLOBAL_LOCK:  # ★ 교차요청 섞임 방지
    ...
    def get_waist_width(self, points):
        ...
        value = self.get_euclidean_length(v_x1, v_y1, v_x2, v_y2)
        point = self.get_middle_point(v_x1, v_y1, v_x2, v_y2)
        self.results.waist_point = point
        return value

    def calculate(self):
        self.results = BodyPartValue()
        for label in np.unique(self.segment):
            if label == 6:   indexes = BodyPart.left_lower_arm()
            elif label == 10: indexes = BodyPart.left_upper_arm()
            elif label == 15: indexes = BodyPart.right_lower_arm()
            elif label == 19: indexes = BodyPart.right_upper_arm()
            elif label == 7:  indexes = BodyPart.left_lower_leg()
            elif label == 11: indexes = BodyPart.left_upper_leg()
            elif label == 16: indexes = BodyPart.right_lower_leg()
            elif label == 20: indexes = BodyPart.right_upper_leg()
            else:             continue

            category, *index = indexes[0], [part for part in indexes[1:]]
            target_points = self.keypoints[index].reshape(-1)
            if -1 in target_points: continue
            ...
            if category == 'arm' or category == 'leg':
                try:
                    ...
                    continue
                point = self.get_middle_point(*target_points)
                if label == 6:
                    self.results.left_lower_arm_width = value; self.results.left_lower_arm_point = point
                elif label == 10:
                    self.results.left_upper_arm_width = value; self.results.left_upper_arm_point = point
                elif label == 15:
                    self.results.right_lower_arm_width = value; self.results.right_lower_arm_point = point
                elif label == 19:
                    self.results.right_upper_arm_width = value; self.results.right_upper_arm_point = point
                elif label == 7:
                    self.results.left_lower_leg_width = value; self.results.left_lower_leg_point = point
                elif label == 11:
                    self.results.left_upper_leg_width = value; self.results.left_upper_leg_point = point
                elif label == 16:
                    self.results.right_lower_leg_width = value; self.results.right_lower_leg_point = point
                elif label == 20:
                    self.results.right_upper_leg_width = value; self.results.right_upper_leg_point = point

        # shoulder
        indexes = BodyPart.shoulder()
        category, *index = indexes[0], [part for part in indexes[1:]]
        target_points = self.keypoints[index].reshape(-1)
        if -1 not in target_points:
            point = self.get_middle_point(*target_points)
            self.results.shoulder_width = self.get_euclidean_length(*target_points)
            self.results.shoulder_point = point
        ...
        if -1 not in target_points:
            self.results.waist_width = self.get_waist_width(target_points)
    ...
    def get_keypoints_error(self, img1):
        ...
        for p, c in zip(prev_keypoints, curr_keypoints):
            p_x, p_y = p; c_x, c_y = c
            d = self.get_euclidean_length(p_x, p_y, c_x, c_y)
            err_sum += d
        error = err_sum / (W * len(prev_keypoints)) * 100
        return error
